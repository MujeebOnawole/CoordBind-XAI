...
def train_epoch(model, train_loader, optimizer, device, coord_weight=1.0):
    """Train for one epoch with joint loss. Returns average total loss."""
    model.train()
    total_loss = 0
    n_batches = 0
...
def validate(model, val_loader, device):
    """Validate model and return logK1 MAE."""
    model.eval()
    all_preds = []
    all_targets = []
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            all_preds.append(logk1_pred.cpu().numpy())
...
def objective(trial: Trial, config, graphs, meta_df, device, node_feature_dim) -> float:
    ...
    coord_weight = trial.suggest_float('coord_weight', *SEARCH_SPACE['coord_weight'], log=True)
    ...
    for fold, (train_fold_idx, val_fold_idx) in enumerate(kfold.split(train_val_indices)):
        ...
        for epoch in range(max_epochs):
            train_loss = train_epoch(model, train_loader, optimizer, device, coord_weight=coord_weight)
