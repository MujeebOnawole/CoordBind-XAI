...
def metal_oxidation_label(metal_type, formal_charge):
    """
    Create a metal label with oxidation state, e.g. 'Fe+2', 'Cr+3', 'Tl+1'.

    Falls back to element-only label if charge is missing/unknown.
    """
    if metal_type in (None, '', 'coord_only'):
        return str(metal_type) if metal_type else 'unknown'
    if formal_charge is None or formal_charge == -999:
        return str(metal_type)
    fc = int(formal_charge)
    if fc > 0:
        return f"{metal_type}+{fc}"
    elif fc < 0:
        return f"{metal_type}{fc}"
    else:
        return f"{metal_type}0"
...
def substructure_masking(model, data, device='cpu'):
    ...
    with torch.no_grad():
        output = model(batch)
        logk1 = output['logk1'] if isinstance(output, dict) else output
        base_pred = logk1.item() if logk1.numel() == 1 else logk1[0, 0].item()
    ...
    for i in range(n_nodes):
        ...
        with torch.no_grad():
            ...
            logk1 = output['logk1'] if isinstance(output, dict) else output
            masked_pred = logk1.item() if logk1.numel() == 1 else logk1[0, 0].item()
...
def get_ensemble_prediction(models, data, device):
    ...
    for model in models:
        ...
        with torch.no_grad():
            output = model(batch)
            logk1 = output['logk1'] if isinstance(output, dict) else output
            pred = logk1.item() if logk1.numel() == 1 else logk1[0, 0].item()
...
class XAIAnalyzer:
    ...
    def analyze_batch(self, graphs, meta_df=None, output_dir=None):
        ...
        for i, data in enumerate(tqdm(graphs, desc="XAI Analysis")):
            try:
                ...
                # Add ID and metal oxidation state
                if meta_df is not None and i < len(meta_df):
                    ...
                    # Extract formal charge from graph object for oxidation-state-aware grouping
                    fc = getattr(data, 'metal_formal_charge', -999)
                    if hasattr(fc, 'item'):
                        fc = fc.item()
                    mt = meta_df.iloc[i].get('metal_type', '')
                    result['metal_ox'] = metal_oxidation_label(mt, fc)
                else:
                    result['idx'] = i
    ...
    def _print_summary(self, results, scenario_counts):
        ...
        # Agreement by metal type (element-only, backward compatible)
        if results and 'metal_type' in results[0]:
            logger.info(f"\nScenario A rate by metal type (element):")
        ...
        # Agreement by metal+oxidation state (new — splits e.g. Fe+2 vs Fe+3)
        if results and 'metal_ox' in results[0]:
            logger.info(f"\nScenario A rate by metal+oxidation state:")
            ox_results = defaultdict(list)
            for r in results:
                ox_results[r['metal_ox']].append(r['scenario'] == 'A')

            # Sort by count descending for readability
            for metal_ox in sorted(ox_results.keys(), key=lambda k: -len(ox_results[k])):
                vals = ox_results[metal_ox]
                rate = sum(vals) / len(vals) * 100
                logger.info(f"  {metal_ox}: {rate:.1f}% Scenario A (n={len(vals)})")
    ...
    def _save_results(self, results, scenario_counts, output_dir):
        ...
        # Per-metal-oxidation Scenario A rates
        ox_scenario_a = {}
        if results and 'metal_ox' in results[0]:
            ox_groups = defaultdict(list)
            for r in results:
                ox_groups[r['metal_ox']].append(r['scenario'] == 'A')
            for k in sorted(ox_groups.keys(), key=lambda k: -len(ox_groups[k])):
                vals = ox_groups[k]
                ox_scenario_a[k] = {
                    'scenario_A_pct': round(sum(vals) / len(vals) * 100, 1),
                    'n': len(vals),
                }
        ...
        summary = {
            ...
            'per_metal_oxidation': ox_scenario_a,
        }
