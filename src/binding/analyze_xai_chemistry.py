...
def _plot_donor_attribution_heatmap(donor_pref, figures_dir):
    ...
    # PRGn: purple = negative attribution (weakens binding), green = positive (strengthens binding).
    # Avoids red and uses the conventional green = good direction for the strengthening signal.
    im = ax.imshow(matrix, cmap='PRGn', vmin=-vmax, vmax=vmax, aspect='auto')
...
def _plot_irving_williams(xai_df, iw_metals, figures_dir):
    ...
    axes[0].set_ylabel('Predicted logK$_1$', fontsize=14)
    ...
    if true_col in iw_data.columns:
        ...
        axes[1].set_ylabel('Experimental logK$_1$', fontsize=14)
...
def _plot_hsab_heatmap(ct_mean, ct_count, figures_dir):
    ...
    vmax = np.nanmax(np.abs(matrix))
    # viridis: dark purple = low logK1, yellow = high logK1.
    # Perceptually uniform, colourblind-safe, no red association with strong binding.
    ...
    for i in range(len(row_order)):
        for j in range(len(col_order)):
            ...
            n = counts[i, j]
            if not np.isnan(val):
                # On viridis, low values are dark (need white text) and high values are bright (need black text).
                color = 'white' if val < 0.5 * np.nanmax(matrix) else 'black'
...
def _plot_lanthanide_contraction(xai_df, figures_dir):
    ...
    for ax_idx, (col, title) in enumerate([('logK1_pred', 'Predicted'),
        ...
        axes[ax_idx].set_ylabel(f'{title} logK$_1$', fontsize=14)
        axes[ax_idx].text(0.02, 0.98, f'({chr(97 + ax_idx)})',
...
def _plot_selectivity(results, figures_dir):
    ...
    ax.set_ylabel('Mean selectivity (Δ logK$_1$)', fontsize=14)
    ax.set_xlabel('Metal pair (first metal favoured if positive)', fontsize=14)
