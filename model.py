"""
model.py - RGCN Model for logK1 Stability Constant Prediction
...
2. Donor-aware triple readout (NEW, motivated by tmGNN-XAI XAI finding):
   ...
   This is analogous to PoreGCN's dual readout (atom pool + pore pool),
   applied to coordination chemistry: donors are the binding interface,
   ligand backbone is the electronic/steric context.

Edges: 4 types (SINGLE, DOUBLE, TRIPLE, METAL_COORD) — parsimonious,
each is a real chemical bond type. No chirality, no distance bins.

Nodes: 100-dim features (element, properties, metal context) + role
labels (metal/donor/ligand) for readout pooling.

node_roles are assigned during graph construction from RDKit dative
bond analysis — donor = non-metal atom with a dative bond to metal.
"""
...
class RGCNStability(nn.Module):
    """
    Relational GCN for logK1 prediction with donor-aware readout.
    ...
    Forward pass:
        ...
        [RGCNConv × N layers, 4 edge types] + residual + LayerNorm
            |
        Triple pooling:
            metal_pool  = mean over metal atom(s)     [role=1]
            donor_pool  = mean over donor atoms        [role=2]
            ligand_pool = mean over other ligand atoms [role=0]
            |
        Concatenate → MLP → logK1
    """
    ...
    def __init__(
        ...
        # Output head: [metal_pool, donor_pool, ligand_pool] → logK1
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        # Triple readout: metal / donor / ligand
    ...
    def _role_pool(self, x, batch, roles, role_id, batch_size):
        ...
        if mask.any():
            ...
            # scatter_mean: efficient batched mean pooling
